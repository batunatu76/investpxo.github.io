                    ...
                    <form action="{{route('search.trans.user')}}" method="Get">
                        <div class="form-row">
                            <div class="form-group col-md-5">
                                <input type="text" class="form-control transub" placeholder="{{__('Search Via Trans ID')}}" name="trans_id" value="{{isset(request()->trans_id) ? request()->trans_id: '' }}">
                            </div>
                            <div class="form-group col-md-5">
                                <select class="form-control transel" name="type">
                            ...
                            <div class="form-group col-md-1">
                                <button type="submit" class="btn btn-base btn-sm btn-block"> <i class="fa fa-search"></i></button>
                            </div>
                        </div>
                    </form>
                </div>
                            ...
                            <div class="tab-pane deposit-tab fade active show" id="home" role="tabpanel" aria-labelledby="home-tab">
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        ...
                                        <tr>
                                            <td>{{__($data->trans_id)}}</td>
                                            <td>{{__($data->description)}}</td>
                                            <td>{{__($data->amount)}}{{__($general->currency)}}</td>
                                            <td>{{__($data->old_bal)}}{{__($general->currency)}}</td>
                                            <td>{{__($data->new_bal)}}{{__($general->currency)}}</td>
                                            <td>
                                                @if($data->status == 0)
                                            ...
                                            <td>{{__(date('d/m/y  h:i A',strtotime($data->created_at)))}}</td>
                                        </tr>
                                        @endforeach
                                        </tbody>
                                    </table>
